...
@Component({
  ...
  animations: [
    trigger('popup', [
      ...
      transition('show => exit', [animate('0.2s ease-out')]),
    ]),
  ],
})
export class CardDeckComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.router.url.endsWith('all')) {
      ...
      this.notifyService.isLoading.subscribe(
        loading => (this.loading = loading),
      );
      this.notifyService.currentRegion.subscribe(region =>
        this.getCardsForRegion(region),
      );
    }
  }
  ...
  private static setActiveRegionCategoryType(
  ...
  public showOnMap(event: MouseEvent, showOnMapObj?: JQuery): void {
    ...
    let poiId: string;
    if (showOnMapObj) {
      poiId = showOnMapObj.attr('show-card')!;
    } else {
      poiId = $(event.target as HTMLElement)
        .closest('*[show-card]')
        .attr('show-card')!;
    }
  ...
  public onCardClick(event: MouseEvent): void {
    ...
    if (!openLink.data() && !showOnMap.data()) {
      ...
      const popupCardId = currentTarget.id;
